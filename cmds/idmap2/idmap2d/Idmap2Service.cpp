...
constexpr std::string_view kPlatformPath = "/system/framework/com.evervolv.platform-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  const bool is_vendor_framework = target_path == kPlatformPath;
  bool use_cache;
  struct stat st = {};
  if (is_framework || is_vendor_framework || !::stat(target_path.c_str(), &st)) {
    use_cache = true;
  } else {
