...
class EdgeLightViewController @Inject constructor(
    ...
    private val settingsObserver = object : ContentObserver(null) {
        override fun onChange(selfChange: Boolean, uri: Uri?) {
            if (DEBUG) Log.d(TAG, "setting changed for ${uri?.lastPathSegment}")
            coroutineScope.launch {
                settingsMutex.withLock {
                    when (uri?.lastPathSegment) {
                        EVSettings.System.EDGE_LIGHT_ENABLED ->
                            edgeLightEnabled = isEdgeLightEnabled()
                        EVSettings.System.EDGE_LIGHT_ALWAYS_TRIGGER_ON_PULSE ->
                            alwaysTriggerOnPulse = alwaysTriggerOnPulse()
                        EVSettings.System.EDGE_LIGHT_REPEAT_ANIMATION ->
                        ...
                        EVSettings.System.EDGE_LIGHT_CUSTOM_COLOR ->
    ...
    private suspend fun isEdgeLightEnabled(): Boolean {
        return withContext(Dispatchers.IO) {
            systemSettings.getIntForUser(EVSettings.System.EDGE_LIGHT_ENABLED, 0, UserHandle.USER_CURRENT) == 1
        }
    }
    ...
    override fun onScreenTurnedOn() {
        ...
        screenOn = true
        if (pulsing) {
            if (DEBUG) Log.d(TAG, "onScreenTurnedOn: pulsing: show()")
            show()
        }
    }
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification, rankingMap: RankingMap) {
        if (DEBUG) Log.d(TAG, "onNotificationPosted, sbn = $sbn")
        coroutineScope.launch {
            settingsMutex.withLock {
        ...
        coroutineScope.launch {
            settingsMutex.withLock {
                if (pulsing && (alwaysTriggerOnPulse ||
                    ...
                    if (screenOn) {
                        if (DEBUG) Log.d(TAG, "setPulsing: screenOn: show()")
                        show()
                    }
                } else {
