...
public class TunerServiceImpl extends TunerService {
    ...
    private boolean isEVSecure(String key) {
        return key.startsWith("evsecure:");
    }
    ...
    public String getValue(String setting) {
        if (isEVGlobal(setting)) {
            ...
            return EVSettings.System.getStringForUser(
                    mContentResolver, chomp(setting), mCurrentUser);
    ...
    public void setValue(String setting, String value) {
        if (isEVGlobal(setting)) {
            ...
            EVSettings.System.putStringForUser(
                    mContentResolver, chomp(setting), value, mCurrentUser);
        } else if (isSystem(setting)) {
    ...
    public int getValue(String setting, int def) {
        if (isEVGlobal(setting)) {
            ...
            return EVSettings.System.getIntForUser(
                    mContentResolver, chomp(setting), def, mCurrentUser);
        } else if (isSystem(setting)) {
    ...
    public String getValue(String setting, String def) {
        ...
        if (isEVGlobal(setting)) {
            ...
            ret = EVSettings.System.getStringForUser(
                    mContentResolver, chomp(setting), mCurrentUser);
        } else if (isSystem(setting)) {
    ...
    public void setValue(String setting, int value) {
        if (isEVGlobal(setting)) {
            ...
            EVSettings.System.putIntForUser(
                    mContentResolver, chomp(setting), value, mCurrentUser);
        } else if (isSystem(setting)) {
    ...
    public void clearAllFromUser(int user) {
        ...
        for (String key : mTunableLookup.keySet()) {
            if (ArrayUtils.contains(RESET_EXCEPTION_LIST, key)
                    || isEVGlobal(key) || isEVSecure(key) || isEVSystem(key)) {
                continue;
            }
            setValue(key, null);
        }
    ...
    private class Observer extends ContentObserver {
        ...
        public void onChange(boolean selfChange, java.util.Collection<Uri> uris,
            ...
            for (Uri u : uris) {
                ...
                if (userId == mUserTracker.getUserId() || isEVGlobal(key)) {
                    reloadSetting(u);
                }
            }
        }
